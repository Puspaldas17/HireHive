...
export function exportToCSV(
  ...
  filename: string = "job_applications.csv",
) {
  // Prepare CSV headers
  const headers = [
    "Company",
 ...
 */
export function exportApplicationReport(
  app: JobApplication,
  filename: string = "application_report.txt",
) {
  const report = `
...
export function exportSummaryReport(
  ...
  const successRate =
    totalApps > 0 ? Math.round((byStatus.Offer / totalApps) * 100) : 0;
